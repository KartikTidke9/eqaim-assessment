...
import { sortFeedbacks } from "../store";

export type Data = { id: number; label: string, value?: string};

const data = [
  { id: 1, label: "Most Upvoted" },
  { id: 2, label: "Least Upvoted" },
  { id: 3, label: "Most Comments" },
  { id: 4, label: "Least Comments" },
];

function FeedbackBar({ numOfFeedbacks }: { numOfFeedbacks: number }) {
  const [sortBy, setSortBy] = useState<Data>(data[0]);
  const navigate = useNavigate();
  const dispatch = useAppDispatch()

  const handleSort = (obj: Data) => {
    setSortBy(obj);
  };

  useEffect(()=>{
    dispatch(sortFeedbacks(sortBy.label))
  },[dispatch, sortBy.label])

  return (
    <div className="flex items-center justify-between p-4 text-sm rounded-lg bg-color-3">
      <div className="flex gap-12">
        <div className="flex items-center gap-4">
          ...
          <span className="font-bold text-color-4">{numOfFeedbacks} <span className="ms-2">Suggestions</span></span>
        </div>

        <span className="relative flex items-center gap-3 opacity-50 select-none text-color-5 hover:cursor-pointer">
          Sort by :{" "}
          <Dropdown
            className="absolute grid w-64 min-w-full grid-cols-1 text-black divide-y rounded-lg shadow-2xl mt-72"
            data={data}
            selectedFieldClasses="flex items-center justify-between gap-3"
            selected={sortBy}
            onItemChange={handleSort}
          />
        </span>
      </div>
      ...
      <Button
        ...
        className="flex items-center justify-center gap-1 px-6 py-2 font-bold rounded-lg text-color-4 bg-color-1 hover:bg-color-11"
